...
| Validation/forms | Valibot (frontend); Zod (backend); type-only API contracts | Validate route params, API responses, and forms at boundaries. The frontend owns Valibot runtime schemas and checks their inferred response types against `@open-care/api-contract` with type-only imports; it must never import backend internals or database modules. |
