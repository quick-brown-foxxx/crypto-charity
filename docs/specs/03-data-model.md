...
`public_beneficiary_ref` is generated by `vault-api-write` after the operator
Worker forwards the disbursement request when omitted from
`POST /api/disbursements` and matches `^benpub_[A-Z2-7]{16}$`. The suffix uses
the RFC 4648 base32 alphabet (`A-Z2-7`), excluding ambiguous `0`, `1`, `8`, and
`9` characters. It
...
This excerpt shows the logical bot conversation shape. Concrete migrations may
add storage-level CHECK constraints as defense-in-depth, but application/API
validation is authoritative for the strict public-ref contract described below.
...
or `handles.opaque_id`; it is a server-generated `^benpub_[A-Z2-7]{16}$`
or `null`. See the cross-DB handoff note in
[`04-api.md`](04-api.md) §"`POST /api/disbursements`".

The canonical application/API validation for this field is the strict RFC 4648
base32 public-ref format. Storage-level checks are defense-in-depth only and
must not be treated as permission for callers to supply broader strings.
